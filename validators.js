...
const updateShippingValidation = [
  ...
  body('houseno').trim().not().isEmpty().withMessage("House Number is required.").matches(/^[0-9A-Za-z.' -]+$/i).withMessage("House number should only contain letters, numbers, periods, dashes, apostrophes and spaces."),
  body('brngy').trim().not().isEmpty().withMessage("Barangay is required.").matches(/^[0-9A-Za-z ]+$/i).withMessage("Barangay should only contain letters, numbers and spaces."),
  body('city').trim().not().isEmpty().withMessage("City is required.").matches(/^[A-Za-z. -]+$/i).withMessage("City should only contain letters, periods, dashes and spaces."),
  body('prov').trim().not().isEmpty().withMessage("Province is required.").matches(/^[A-Za-z ]+$/i).withMessage("Province should only contain letters and spaces."),
];
