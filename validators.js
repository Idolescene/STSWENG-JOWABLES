...
const updateShippingValidation = [
  ...
  body('houseno').trim().not().isEmpty().withMessage("House Number is required.").matches(/^[0-9A-Za-z.' ,-]+$/i).withMessage("House number should only contain letters, numbers, periods, dashes, apostrophes, commas and spaces.").not().matches(/^NONE$/i).withMessage("House Number should not be NONE."),
